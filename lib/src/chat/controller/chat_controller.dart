    ...
    if (promptController.text.isNotEmpty || _isSeaching) {
      ...
      try {
        ...
        var response = await baseDio.post(
            'v1beta3/models/text-bison-001:generateText?key=${Config.botChatApiKey}',
            body: {
              "prompt": {"text": prompt},
              "temperature": 1.0,
            });
        ...
        if (response.statusCode == 200) {
          ...
          await chatRepository.create(ChatModel(
            prompt: _output,
            isMe: false,
          ));
          await chatRepository.refresh();
        ...
        setIsSeaching(false);
        setHasError(
            value: true, error: '${e.response?.data['error']['message']}');
      }
    } else {
      return null;
    }
